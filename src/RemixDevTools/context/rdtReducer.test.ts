...
describe("rdtReducer", () => {
  it("should return the initial state", () => {
    expect(rdtReducer(undefined, {} as any)).toEqual(initialState);
  });
  ...
  it("should handle SET_SETTINGS", () => {
    ...
    expect(rdtReducer(initialState, { type: "SET_SETTINGS", payload })).toEqual(
      expectedState
    );
  });
  ...
  it("should not clear any output if terminal doesn't exist", () => {
    const expectedState = {
      ...initialState,
      terminals: [
        { ...terminal, output: [{ type: "command", value: "test" }] },
      ],
    };
    expect(
      rdtReducer(
