export function removeStyleAndDataAttributes(inputString: string) {
  ...
  let resultString = inputString.replaceAll(styleTagRegex, "").replaceAll(scriptTagRegex, "")
  .replaceAll(templateRegex, "")
  .replaceAll("<!--$?-->", "")
  .replaceAll("<!--/$-->", "");  
  
  resultString = resultString.replaceAll(styleRegex, function(match, styleValue) {
...
export const hydrationDetector = (isVite: boolean) => {
  if (isVite) {
